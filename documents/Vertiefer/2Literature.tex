...
The examined adsorbate \acf{QA} is a high symmetric, prochiral organic molecule with the IUPAC name 5,12-dihydro-quino[2,3-\textit{b}]acridine-7,14-dione and the sum formula \ce{C20H12N2O2}. Organic crystals exhibit a color which can range from red to violet.\autocite{ChemicalBook2025,ChemSpider2025}
...
For \ac{QA} deposited onto the Ag(100) surface, two structural phases were observed to form: the $\alpha$- and the $\beta$-phase. A thorough examination of these two phases has been conducted by N. Humberg.\autocite{Humberg2024, Humberg2020}

The formation of the $\alpha$-phase occurs when the \ac{QA} molecules are deposited onto the Ag(100) surface at a sample temperature of 300~\si{K}. This phase is composed of parallel molecular chains that are flat-lying with the $\pi$-system being oriented parallel to the Ag(100) surface. The \ac{QA} molecules are connected via intermolecular hydrogen bonds between the keto groups of one molecule and the amine groups of another molecule, resulting in a total of 4 hydrogen bonds. It was determined that the domains under consideration are homochiral, a finding that suggests a high degree of mobility for the \ac{QA} molecules on Ag(100) surface at 300~\si{K}.

In addition, the superstructure matrix of the $\alpha$-phase for a complete \ac{ML} was determined using a \ac{SPA-LEED} pattern and \ac{STM} results. The following superstructure matrix was found
\begin{equation*}
...
The \ac{STM} images, the \ac{SPA-LEED} pattern and the current structure model of \ac{QA} on the Ag(100) surface in the $\alpha$-phase is illustrated in \autoref{fig:literature_alpha}.
\begin{figure}[htb]
	\centering
	\includegraphics[width=0.8\textwidth]{images/literature_alpha.png}
...
Once more, employing a \ac{SPA-LEED} pattern and \ac{STM} results, the superstructure matrix of the $\beta$-phase for a complete \ac{ML} was determined as
\begin{equation*}
...
The current structure model proposed by N. Humberg\autocite{Humberg2020,Humberg2024} already allow for the formulation of a prediction of the \ac{XPS} spectra. This prediction is based on the different molecular arrangements and interactions in the $\alpha$-phase and $\beta$-phase. In sum, the adsorbate-substrate interaction in the $\beta$-phase should be stronger than in the $\alpha$-phase, leading to lower \acp{BE} for all components in the $\beta$-phase.
